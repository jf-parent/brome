...
from brome.core.configurator import get_config_value, parse_brome_config_from_browser_config, default_config, test_config_to_dict
from brome.model.test_instance import TestInstance
from brome.model.test_result import TestResult
from brome.model.test_crash import TestCrash
...
class BaseTest(object):
    ...
    def end(self):
        ...
        with DbSessionContext(self.get_config_value('database:mongo_database_name')) as session:
            test_instance =  session.query(TestInstance).filter(TestInstance.mongo_id == self._test_instance_id).one()
    ...
    def create_crash_report(self, tb):
        ...
        if self._browser_config.get('enable_proxy'):
            extra_data['network_capture_path'] = os.path.join(self._network_capture_relative_dir, string_to_filename('%s.data'%self._name))
        ...
        with DbSessionContext(self.get_config_value('database:mongo_database_name')) as session:
            ...
            test_crash.videocapture_path = self._video_capture_file_relative_path
    ...
    def get_test_result_summary(self):
        ...
        with DbSessionContext(self.get_config_value('database:mongo_database_name')) as session:
            ...
            for failed_test in failed_tests:
                query = session.query(Test).filter(Test.mongo_id == failed_test.test_id)
                if query.count():
                    test = query.one()
