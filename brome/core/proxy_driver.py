...
from brome.model.test_quality_screenshot import TestQualityScreenshot
...
class ProxyDriver(object):
    ...
    def create_test_result(self, testid, result, **kwargs):
        ...
        Keyword Args:
            extra_data (dict): the extra data that will be saved with the test result
        ...
        if self.browser_config.get('enable_proxy'):
            extra_data['network_capture_path'] = os.path.join(self.test_instance._network_capture_relative_dir, string_to_filename('%s.data'%self.test_instance._name))
